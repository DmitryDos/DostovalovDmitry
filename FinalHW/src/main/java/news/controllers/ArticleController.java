...
import news.controllers.request.ArticleUpdateRequest;
import news.controllers.request.CommentCreateRequest;
import news.controllers.request.MultiArticleCreateRequest;
...
public class ArticleController implements Controller {
  ...
  public ArticleController(Service service, ArtAndComService articleService, ObjectMapper objectMapper) {
  ...
  private void createArticle() {
    service.post(
        ...
        (Request request, Response response) -> {
          ...
          try {
            ArticleId articleId = articleService.createArticle(articleCreateRequest.getName(), articleCreateRequest.getTags());
  ...
  private void multiCreateArticle() {
    service.post(
        "/api/articles",
        (Request request, Response response) -> {
          response.type("application/json");
          String body = request.body();
          MultiArticleCreateRequest multiArticleCreateRequest = objectMapper.readValue(
              body,
              MultiArticleCreateRequest.class);
          try {
            List<ArticleId> articlesId = articleService.multiCreateArticle(multiArticleCreateRequest.getData());

            LOG.debug("Create articles successfully. Count of articles: {}", articlesId.size());
            response.status(200);
            return objectMapper.writeValueAsString(new MultiArticleCreateResponse(articlesId));
          } catch (ArticleIdDuplicatedException e) {

            LOG.warn("Cannot create articles", e);
            response.status(400);
            return objectMapper.writeValueAsString(new ErrorResponse(e.getMessage()));
          }
        }
  ...
  private void updateArticle() {
    service.post(
        ...
        (Request request, Response response) -> {
          ...
          String body = request.body();
          ArticleCreateRequest articleUpdateRequest = objectMapper.readValue(
              body,
              ArticleCreateRequest.class);
          try {
            if (articleUpdateRequest.getName() != null) {
              articleService.updateArticleWithName(articleId, articleUpdateRequest.getName());
            }
            if (articleUpdateRequest.getTags() != null) {
              articleService.updateArticleWithTags(articleId, articleUpdateRequest.getTags());
            }
